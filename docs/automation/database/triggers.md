...
По сути, значение этого поля является частью топика [`fd`](/common/topics/#fd-from-device), относящейся к конкретному устройству.
...
Если это поле имеет значение `true`, триггер будет игнорировать переход значения через порог и срабатывать в каждом случае, когда полученное значение соответствует заданным критериям, даже если оно не изменилось. Опция работает для всех операторов, кроме `changes` и `updates`.

!!! warning ""

    Использование опции `force` может приводить к нежелательным срабатываниям автоматизации. Будьте внимательны и используйте эту опцию только в случаях, когда она действительно нужна.
