...
| `contains`      | левая часть содержит в себе правую часть
| `excludes`      | левая часть _не_ содержит в себе правую часть
...
!!! info ""
    ...
    Условные операторы `contains` и `excludes` позволяют использовать регулярные выражения в правой части шаблона.

!!! warning ""

    Условные операторы `>`, `>=`, `<` и `<=` применимы только к числовым значениям, результат сравнения строк может оказаться непредсказуемым.
