...
Эта команда позволяет повторно произвести интервью устройства, как это делается при добавлении устройства в сеть.

Пример сообщения:

```json
{
  "action": "interviewDevice",
...
Эта команда позволяет определить адреса и каналы устройств, находящихся в непосредственной близости к координатору, при помощи технологии _TouchLink_, если координатор и устройства ее поддерживают.
