...
Если значение является массивом, можно получать его элементы по их индексу (работает только для одномекрных массивов):
...
В случае, когда какие-то данные устройства могут оказаться некорректными, можно использовать условный шаблон `if/else`, возвращающий специальное “невалидное” значение `_NULL_`. Такое значение будет игнорироваться, и значение соответствующего свойства не изменится. Например:

```
{{ json.humidity if json.humidity is number else _NULL_ }}
```
